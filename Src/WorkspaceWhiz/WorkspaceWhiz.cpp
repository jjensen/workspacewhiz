...
extern void UnregisterAllCommands90(CComPtr<EnvDTE80::DTE2>& pDTE);

void ConnectAndUnregisterAllCommands90()
{
	// Create the shell and get an interface to it.
	CComPtr<EnvDTE80::DTE2> pDTE;
	pDTE.CoCreateInstance(CComBSTR("VisualStudio.DTE.9.0"), 0, CLSCTX_ALL);
//	HRESULT hr = CoCreateInstance(__uuidof(EnvDTE80::DTE2), NULL, CLSCTX_LOCAL_SERVER,
//		__uuidof(EnvDTE80::DTE2), reinterpret_cast<void**>(&pDTE));
	if (pDTE)
	{
		UnregisterAllCommands90(pDTE);
...
extern void UnregisterAllCommands100(CComPtr<EnvDTE80::DTE2>& pDTE);

void ConnectAndUnregisterAllCommands100()
{
	// Create the shell and get an interface to it.
	CComPtr<EnvDTE80::DTE2> pDTE;
	pDTE.CoCreateInstance(CComBSTR("VisualStudio.DTE.10.0"), 0, CLSCTX_ALL);
//	HRESULT hr = CoCreateInstance(__uuidof(EnvDTE80::DTE2), NULL, CLSCTX_LOCAL_SERVER,
//		__uuidof(EnvDTE80::DTE2), reinterpret_cast<void**>(&pDTE));
	if (pDTE)
	{
		UnregisterAllCommands100(pDTE);
...
extern void UnregisterAllCommands110(CComPtr<EnvDTE80::DTE2>& pDTE);

void ConnectAndUnregisterAllCommands110()
{
	// Create the shell and get an interface to it.
	CComPtr<EnvDTE80::DTE2> pDTE;
	pDTE.CoCreateInstance(CComBSTR("VisualStudio.DTE.11.0"), 0, CLSCTX_ALL);
//	HRESULT hr = CoCreateInstance(__uuidof(EnvDTE80::DTE2), NULL, CLSCTX_LOCAL_SERVER,
//		__uuidof(EnvDTE80::DTE2), reinterpret_cast<void**>(&pDTE));
	if (pDTE)
	{
		UnregisterAllCommands110(pDTE);
...
extern void UnregisterAllCommands120(CComPtr<EnvDTE80::DTE2>& pDTE);

void ConnectAndUnregisterAllCommands120()
{
	// Create the shell and get an interface to it.
	CComPtr<EnvDTE80::DTE2> pDTE;
	pDTE.CoCreateInstance(CComBSTR("VisualStudio.DTE.12.0"), 0, CLSCTX_ALL);
	//	HRESULT hr = CoCreateInstance(__uuidof(EnvDTE80::DTE2), NULL, CLSCTX_LOCAL_SERVER,
	//		__uuidof(EnvDTE80::DTE2), reinterpret_cast<void**>(&pDTE));
	if (pDTE)
	{
		UnregisterAllCommands120(pDTE);
...
//				devKey.SetStringValue("SatelliteDLLName", moduleShortName);
	...
	if (devKey.Open(HKEY_LOCAL_MACHINE, s_regBasePath + "\\9.0") == ERROR_SUCCESS)
	{
		// Auto create the addins key if it isn't already there.
		if (devKey.Create(HKEY_LOCAL_MACHINE, s_regBasePath + "\\9.0\\AddIns") == ERROR_SUCCESS)
		{
			// Create the WorkspaceWhiz.DSAddin.1 key.
			if (devKey.Create(HKEY_LOCAL_MACHINE, s_regBasePath + "\\9.0\\AddIns\\WWhizNet.Connect") == ERROR_SUCCESS)
			{
				// Remove all old entries.
				devKey.SetStringValue("SatelliteDLLPath", modulePath);
//				devKey.SetStringValue("SatelliteDLLName", moduleShortName);
	...
	if (devKey.Open(HKEY_CURRENT_USER, s_regBasePath + "\\9.0\\PreloadAddinState") == ERROR_SUCCESS)
	{
		devKey.SetDWORDValue("WWhizNet.Connect", 1);
	}

	if (devKey.Open(HKEY_LOCAL_MACHINE, s_regBasePath + "\\10.0") == ERROR_SUCCESS)
	{
		// Auto create the addins key if it isn't already there.
		if (devKey.Create(HKEY_LOCAL_MACHINE, s_regBasePath + "\\10.0\\AddIns") == ERROR_SUCCESS)
		{
			// Create the WorkspaceWhiz.DSAddin.1 key.
			if (devKey.Create(HKEY_LOCAL_MACHINE, s_regBasePath + "\\10.0\\AddIns\\WWhizNet.Connect") == ERROR_SUCCESS)
			{
				// Remove all old entries.
				devKey.SetStringValue("SatelliteDLLPath", modulePath);
//				devKey.SetStringValue("SatelliteDLLName", moduleShortName);
	...
	if (devKey.Open(HKEY_CURRENT_USER, s_regBasePath + "\\10.0\\PreloadAddinState") == ERROR_SUCCESS)
	{
		devKey.SetDWORDValue("WWhizNet.Connect", 1);
	}

	if (devKey.Open(HKEY_CURRENT_USER, s_regBasePath + "\\11.0") == ERROR_SUCCESS)
	{
		// Auto create the addins key if it isn't already there.
		if (devKey.Create(HKEY_CURRENT_USER, s_regBasePath + "\\11.0\\AddIns") == ERROR_SUCCESS)
		{
			// Create the WorkspaceWhiz.DSAddin.1 key.
			if (devKey.Create(HKEY_CURRENT_USER, s_regBasePath + "\\11.0\\AddIns\\WWhizNet.Connect") == ERROR_SUCCESS)
			{
				// Remove all old entries.
				devKey.SetStringValue("SatelliteDLLPath", modulePath);
//				devKey.SetStringValue("SatelliteDLLName", moduleShortName);
	...
	if (devKey.Open(HKEY_CURRENT_USER, s_regBasePath + "\\12.0") == ERROR_SUCCESS)
	{
		// Auto create the addins key if it isn't already there.
		if (devKey.Create(HKEY_CURRENT_USER, s_regBasePath + "\\12.0\\AddIns") == ERROR_SUCCESS)
		{
			// Create the WorkspaceWhiz.DSAddin.1 key.
			if (devKey.Create(HKEY_CURRENT_USER, s_regBasePath + "\\12.0\\AddIns\\WWhizNet.Connect") == ERROR_SUCCESS)
			{
				// Remove all old entries.
				devKey.SetStringValue(_T("SatelliteDLLPath"), modulePath);
				//				devKey.SetStringValue("SatelliteDLLName", moduleShortName);
				devKey.SetStringValue(_T("SatelliteDLLName"), _T("WWhizResources.dll"));
				devKey.SetDWORDValue(_T("LoadBehavior"), 3);
				devKey.SetStringValue(_T("FriendlyName"), _T("Workspace Whiz - A Visual Studio Add-in"));
				devKey.SetStringValue(_T("Description"), _T("Adds useful code navigation and completion capabilities to Visual Studio"));
			}
		}
	}

	if (devKey.Open(HKEY_CURRENT_USER, s_regBasePath + "\\12.0\\PreloadAddinState") == ERROR_SUCCESS)
	{
		devKey.SetDWORDValue(_T("WWhizNet.Connect"), 1);
	}
#endif WWHIZ_VSNET
...
STDAPI DllUnregisterServer(void)
{
	...
	if (key.Open(HKEY_LOCAL_MACHINE, s_regBasePath + "\\9.0\\AddIns") == ERROR_SUCCESS)
	{
		// Remove all old entries.
		key.RecurseDeleteKey("WWhizNet.Connect");
	}

	if (key.Open(HKEY_LOCAL_MACHINE, s_regBasePath + "\\10.0\\AddIns") == ERROR_SUCCESS)
	{
		// Remove all old entries.
		key.RecurseDeleteKey("WWhizNet.Connect");
	}

	if (key.Open(HKEY_LOCAL_MACHINE, s_regBasePath + "\\11.0\\AddIns") == ERROR_SUCCESS)
	{
		// Remove all old entries.
		key.RecurseDeleteKey("WWhizNet.Connect");
	}

	if (key.Open(HKEY_LOCAL_MACHINE, s_regBasePath + "\\12.0\\AddIns") == ERROR_SUCCESS)
	{
		// Remove all old entries.
		key.RecurseDeleteKey("WWhizNet.Connect");
	}
...
/*			if (key.Open(HKEY_CURRENT_USER, "Software") == ERROR_SUCCESS)
	...
	ConnectAndUnregisterAllCommands90();
	ConnectAndUnregisterAllCommands100();
	ConnectAndUnregisterAllCommands110();
	ConnectAndUnregisterAllCommands120();
