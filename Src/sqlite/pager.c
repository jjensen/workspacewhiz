...
/*
** Write the database given by "pPager" to file "szFilename".
*/
int sqlite3pager_write_to_file(Pager* pPager, const char* szFilename)
{
  OsFile   *pFd;
  OsFile   *fd;
  OsFile   *oldFd;
  ...
  if ((rc = sqlite3OsOpenReadWrite(szFilename, &fd, &readOnly)) != SQLITE_OK)
    return rc;

  /* save old filedescriptor */
  oldFd = pPager->fd;
  /* switch from memoryDB to diskDB */
  pPager->fd    = fd;
  pPager->memDb = 0;
  
  if ((rc = sqlite3OsLock(pPager->fd, SHARED_LOCK)) != SQLITE_OK)
    goto restore_pager;

  /* mark all pages as dirty, so they will be written to disk */
  for (pPg = pPager->pAll; pPg; pPg = pPg->pNextAll)
    makeDirty(pPg);

  if ((pPg = pager_get_all_dirty_pages(pPager)) == NULL)
  {
    rc = SQLITE_INTERNAL;
    goto restore_pager;
  }
  
  if ((rc  = pager_write_pagelist(pPg)) != SQLITE_OK)
    goto restore_pager;
